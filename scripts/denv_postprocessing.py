...
#Get list of all compound-denv viral protein interactions 
compound_viral_df = pd.read_csv("../data/DENV/denv_Compound_Viral_interactions_for_Supervised_Learning_full_metadata.csv",header='infer')
print("Loaded compound viral protein interactions for DENV viral proteins")
...
def get_ranked_list(df,proteins,rev_drug_info,protein_mapping_dict,ranked_list_proteins):
    for i in range(len(proteins)):
        ...
        #Subset to single denv viral protein
        ...
        #Subset to the same single denv viral protein
...
# def per_protein_rank_ano(ranked_list_proteins, protein_name):
...
def per_protein_rank(ranked_list_proteins, protein_name):
    ...
    for rank_list in temp_list:
        for idx, drug in enumerate(rank_list):
            score_dict[drug] += idx + 1  # 排名从1开始
            count_dict[drug] += 1
    # 计算平均排名
    avg_rank = {drug: score_dict[drug] / count_dict[drug] for drug in score_dict}
    # 按平均排名升序排序
    sorted_drugs = sorted(avg_rank.items(), key=lambda x: x[1])
    # 返回格式与原rankaggregation类似
    return [(drug, rank) for drug, rank in sorted_drugs]

# +
cnn_predictions = pd.read_csv("../results/cnn_supervised_denv_predictions.csv",header='infer',sep=",")
lstm_predictions = pd.read_csv("../results/lstm_supervised_denv_predictions.csv",header='infer',sep=",")
cnn_lstm_predictions = pd.read_csv("../results/cnn_lstm_supervised_denv_predictions.csv",header='infer',sep=",")
gat_cnn_predictions = pd.read_csv("../results/gat_cnn_supervised_denv_predictions.csv",header='infer',sep=',')

#Get a list of the unique proteins
# all_proteins = rf_smiles_predictions["uniprot_accession"].unique()
all_proteins = cnn_predictions["uniprot_accession"].unique()
...
#Get ranked list for each protein using ML methods
...
def combined_df(df1,df2,df3,df4,protein_id):
...
def get_results_with_pchembl(final_combined_df,rev_drug_info,protein_name):
    ...
    average_combined_df["avg_predictions"]=final_combined_df.iloc[:,[2,3,4,5]].mean(axis=1)
...
#For NS1
final_combined_df = combined_df(cnn_predictions,lstm_predictions,cnn_lstm_predictions,gat_cnn_predictions,all_proteins[0])
output1_df = get_results_with_pchembl(final_combined_df,compound_viral_df,protein_mapping_dict[all_proteins[0]])
output1_df.to_csv("../results/NS1_Pro_Top_Ranked_Compounds.csv",index=False)
print("Ranked list of top compounds for NS1 written")

#For NS3
final_combined_df = combined_df(cnn_predictions,lstm_predictions,cnn_lstm_predictions,gat_cnn_predictions,all_proteins[1])
output2_df = get_results_with_pchembl(final_combined_df,compound_viral_df,protein_mapping_dict[all_proteins[1]])
output2_df.to_csv("../results/NS3_Pro_Top_Ranked_Compounds.csv",index=False)
print("Ranked list of top compounds for NS3 written")

#For NS5
final_combined_df = combined_df(cnn_predictions,lstm_predictions,cnn_lstm_predictions,gat_cnn_predictions,all_proteins[2])
output3_df = get_results_with_pchembl(final_combined_df,compound_viral_df,protein_mapping_dict[all_proteins[2]])
output3_df.to_csv("../results/NS5_Pro_Top_Ranked_Compounds.csv",index=False)
print("Ranked list of top compounds for NS5 written")
